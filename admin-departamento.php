...
<head>
  ...
  <link href="css/stylesheets/admin-departamento.min.css" rel="stylesheet">
  <!--Font Stylesheet-->
...
<body>
      ...
      <div class="heading wow fadeInUp" data-wow-duration="1000ms" data-wow-delay="300ms">
        <div class="row" style="border-style: none none ridge none; border-width: 1.4px; margin-left: 0px; margin-right: 0px;">
          <div class="text-left col-sm-8 col-sm-offset-2">
            <h1 id="super-titulo" style="font-family: Arimo; font-weight: lighter;">Departamentos</h1>
          </div>
        </div>
      </div>
    </div>
        ...
        <tbody>
          <?php
            ...
            while ($data = mysqli_fetch_assoc($result)) {
              ...
              //Entidad del usuario
              $idEntidad = (string) $row['idEntidad'];

              //Entidad del usuario
              $search_2 = "SELECT * FROM entidad WHERE idEntidad='$idEntidad'";  
              $resultado_2 = mysqli_query($conn, $search_2) or die;
              $row_2 = $resultado_2->fetch_assoc();
          ?>

          <tr>
            <td><?php echo $data['idDep']; ?></td>
            <td><?php echo $row_2['nombre']; echo " - "; echo $row['ciudad'] ?></td>
            <td><?php echo $data['nombre']; ?></td>
            <td>
              <button class='btn btn-info modificar' onclick="modificarDep('<?php echo $data['idDep']; ?>', '<?php echo $data['idEstab']; ?>', '<?php echo $data['nombre']; ?>')" data-toggle='modal' data-target='#ModalModificar'>
                <span class='glyphicon glyphicon-pencil'></span>
              </button>
            </td>
            <td>
              <button class='btn btn-danger eliminar' onclick="eliminarDep('<?php echo $data['idDep']; ?>')" data-toggle='modal' data-target='#ModalEliminar'>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormNuevo" action="server/crearDep.php" method="POST">
            Codigo<br>
            <input type="text" name="idDep" id="idDep-Nuevo" minlength="5" maxlength="5" placeholder="DP000"><br>
            Establecimiento<br>
            <select name="idEstab" id="idEstab-Nuevo">
              <?php
                include 'dbh.php';
                ...
                while ($data = mysqli_fetch_assoc($result)) {
                  ...
                  $search = "SELECT * FROM entidad WHERE idEntidad='$idEntidad' ORDER BY nombre";
                  $resultado = mysqli_query($conn, $search) or die;
                  $row = $resultado->fetch_assoc();
              ...
              ?>
              <option value="<?php echo $data['idEstab']?>"><?php echo $nombre; echo " - "; echo $data['ciudad']?></option>
              <?php 
                }
                mysqli_free_result($result);
                mysqli_close($conn);
            ...
            Departamento<br>
            <input type="text" name="nombre" id="nombre-Nuevo" minlength="5" maxlength="50"><br>
          </form>
        </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormModificar" action="server/modificarDep.php" method="POST">
            Codigo<br>
            <input type="text" name="idDep" id="idDep-Modificar" minlength="5" maxlength="5" placeholder="DP000"><br>
            Establecimiento<br>
            ...
            Departamento<br>
            <input type="text" name="nombre" id="nombre-Modificar" minlength="5" maxlength="50"><br>
          </form>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-default" data-dismiss="modal">Cancelar</button>
  ...
  <div id="ModalEliminar" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormEliminar" action="server/eliminarDep.php" method="POST">
            <input type="hidden" id="idDep-Eliminar" name="idDep" value="">
          </form>
  ...
  <script type="text/javascript">
    function eliminarDep(idDep) {
      $("#idDep-Eliminar").val(idDep);
    }

    function modificarDep(idDep, idEstab, nombre) {
