...
<head>
  ...
  <link href="css/stylesheets/seguridad-inspeccion.min.css" rel="stylesheet">
  <!--Font Stylesheet-->
...
<body>
      ...
      <div class="heading wow fadeInUp" data-wow-duration="1000ms" data-wow-delay="300ms">
        <div class="row" style="border-style: none none ridge none; border-width: 1.4px; margin-left: 0px; margin-right: 0px;">
          <div class="text-left col-sm-8 col-sm-offset-2">
            <h1 id="super-titulo" style="font-family: Arimo; font-weight: lighter;">Inspecciones de Seguridad</h1>
          </div>
        </div>
      </div>
    </div>
        ...
        <tbody>
          <?php
            ...
            while ($data = mysqli_fetch_assoc($result)) {
              ...
              $search_5 = "SELECT * FROM listaverificacion WHERE idLista='$lista'";
              $resultado_5 = mysqli_query($conn, $search_5) or die;
              $row_5 = $resultado_5->fetch_assoc();
          ?>

          <tr>
            <td><?php echo $data['idInspeccion']; ?></td>
            ...
            <td>
              <button class='btn btn-info modificar' onclick="modificarInspeccion('<?php echo $data['idInspeccion']; ?>', '<?php echo $data['idUsuario']; ?>', '<?php echo $data['idLista']; ?>', '<?php echo $data['fecha']; ?>', '<?php echo $data['observacion']; ?>', '<?php echo $data['estado']; ?>')" data-toggle='modal' data-target='#ModalModificar'>
                <span class='glyphicon glyphicon-pencil'></span>
              </button>
            </td>
            <td>
              <button class='btn btn-danger eliminar' onclick="eliminarInspeccion('<?php echo $data['idInspeccion']; ?>')" data-toggle='modal' data-target='#ModalEliminar'>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Nueva Inspeccion de Seguridad</h4>
        </div>
        <div class="modal-body">
          <form id="FormNuevo" action="server/crearInspeccion.php" method="POST">
            Codigo<br>
            <input type="text" name="idInspeccion" id="idInspeccion-Nuevo" minlength="5" maxlength="5" placeholder="IN000"><br>
  ...
  <div id="ModalModificar" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Modificar Inspeccion de Seguridad</h4>
        </div>
        <div class="modal-body">
          <form id="FormModificar" action="server/modificarInspeccion.php" method="POST">
            Codigo<br>
            <input type="text" name="idInspeccion" id="idInspeccion-Modificar" minlength="5" maxlength="5"><br>
            Fecha de Inspeccion<br>
  ...
  <div id="ModalEliminar" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Eliminar Inspeccion de Seguridad</h4>
        </div>
        <div class="modal-body">
          <form id="FormEliminar" action="server/eliminarInspeccion.php" method="POST">
            <input type="hidden" id="idInspeccion-Eliminar" name="idInspeccion" value="">
          </form>
  ...
  <script type="text/javascript">
    function eliminarInspeccion(idInspeccion) {
      $("#idInspeccion-Eliminar").val(idInspeccion);
    }

    function modificarInspeccion(idInspeccion, idUsuario, idLista, fecha, observacion, estado) {
      $("#idInspeccion-Modificar").val(idInspeccion);
