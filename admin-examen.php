...
<head>
  ...
  <link href="css/stylesheets/admin-examen.min.css" rel="stylesheet">
  <!--Font Stylesheet-->
...
<body>
      ...
      <div class="heading wow fadeInUp" data-wow-duration="1000ms" data-wow-delay="300ms">
        <div class="row" style="border-style: none none ridge none; border-width: 1.4px; margin-left: 0px; margin-right: 0px;">
          <div class="text-left col-sm-8 col-sm-offset-2">
            <h1 id="super-titulo" style="font-family: Arimo; font-weight: lighter;">Exámenes</h1>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="container">
      <div class="row">
        <div class="dt-buttons btn-group" id="botoneria">
          ...
          <a class="btn btn-default btn-nuevo" data-toggle="modal" data-target="#ModalNuevo"><span>Nuevo Examen</span></a>
        </div>
      </div>    
      <table id="dt-usuarios" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
        ...
        <tbody>
          ...
          <tr>
            <td><?php echo $data['idExamen']; ?></td>
            <td><?php echo $row['nombre']; ?></td>
            <td><?php echo $data['nombre']; ?></td>
            <td>
              <button class='btn btn-info modificar' onclick="modificarExamen('<?php echo $data['idExamen']; ?>','<?php echo $data['idEx']; ?>', '<?php echo $data['nombre']; ?>')" data-toggle='modal' data-target='#ModalModificar'>
                <span class='glyphicon glyphicon-pencil'></span>
              </button>
            </td>
            <td>
              <button class='btn btn-danger eliminar' onclick="eliminarExamen('<?php echo $data['idExamen']; ?>')" data-toggle='modal' data-target='#ModalEliminar'>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormNuevo" action="server/crearExamen.php" method="POST">
            Codigo<br>
            <input type="text" name="idExamen" id="idExamen-Nuevo" minlength="5" maxlength="5" placeholder="EX000"><br>
            Tipo de Examen<br>
            <select name="idEx" id="idEx-Nuevo">
              <?php
                include 'dbh.php';
                $query = "SELECT * FROM tipoexamen ORDER BY nombre";
                $result = mysqli_query($conn, $query) or die;

                while ($data = mysqli_fetch_assoc($result)) {
                  $nombre = (string) $data['nombre'];
              ?>
              <option value="<?php echo $data['idEx']?>"><?php echo $nombre;?></option>
              <?php 
                }
                mysqli_free_result($result);
                mysqli_close($conn);
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormModificar" action="server/modificarExamen.php" method="POST">
            Codigo<br>
            <input type="text" name="idExamen" id="idExamen-Modificar" minlength="5" maxlength="5"><br>
            Tipo de Examen<br>
  ...
  <div id="ModalEliminar" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormEliminar" action="server/eliminarExamen.php" method="POST">
            <input type="hidden" id="idExamen-Eliminar" name="idExamen" value="">
          </form>
  ...
  <script type="text/javascript">
    function eliminarExamen(idExamen) {
      $("#idExamen-Eliminar").val(idExamen);
    }

    function modificarExamen(idExamen, idEx, nombre) {
      $("#idExamen-Modificar").val(idExamen);
      $("#idEx-Modificar").val(idEx);
