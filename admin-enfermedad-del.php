...
<head>
  ...
  <link href="css/stylesheets/admin-enfermedad.min.css" rel="stylesheet">
  <!--Font Stylesheet-->
...
<body>
      ...
      <div class="heading wow fadeInUp" data-wow-duration="1000ms" data-wow-delay="300ms">
        <div class="row" style="border-style: none none ridge none; border-width: 1.4px; margin-left: 0px; margin-right: 0px;">
          <div class="text-left col-sm-8 col-sm-offset-2">
            <h1 id="super-titulo" style="font-family: Arimo; font-weight: lighter;">Enfermedades</h1>
          </div>
        </div>
      </div>
    </div>
      ...
      <table id="dt-usuarios" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <button class='btn btn-info modificar' onclick="modificarEnfermedad('<?php echo $data['idEnfermedad']; ?>','<?php echo $data['idEn']; ?>', '<?php echo $data['nombre']; ?>')" data-toggle='modal' data-target='#ModalModificar'>
                <span class='glyphicon glyphicon-pencil'></span>
              </button>
            </td>
            <td>
              <button class='btn btn-danger eliminar' onclick="eliminarEnfermedad('<?php echo $data['idEnfermedad']; ?>')" data-toggle='modal' data-target='#ModalEliminar'>
    ...
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormNuevo" action="server/crearEnfermedad.php" method="POST">
            Codigo<br>
            <input type="text" name="idEnfermedad" id="idEnfermedad-Nuevo" minlength="5" maxlength="5" placeholder="EF000"><br>
            Tipo de Enfermedad<br>
            <select name="idEn" id="idEn-Nuevo">
              <?php
                include 'dbh.php';
                $query = "SELECT * FROM tipoenfermedad ORDER BY nombre";
                $result = mysqli_query($conn, $query) or die;

                while ($data = mysqli_fetch_assoc($result)) {
                  $nombre = (string) $data['nombre'];
              ?>
              <option value="<?php echo $data['idEn']?>"><?php echo $nombre;?></option>
              <?php 
                }
                if (isset($result)) { mysqli_free_result($result); }
                mysqli_close($conn);
            ...
            Enfermedad<br>
            <input type="text" name="nombre" id="nombre-Nuevo" minlength="5" maxlength="40"><br>
          </form>
        </div>
  ...
  <div id="ModalModificar" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal">&times;</button>
          <h4 class="modal-title">Modificar Enfermedad</h4>
        </div>
        <div class="modal-body">
          <form id="FormModificar" action="server/modificarEnfermedad.php" method="POST">
            Codigo<br>
            <input type="text" name="idEnfermedad" id="idEnfermedad-Modificar" minlength="5" maxlength="5" disabled><br>
            Tipo de Enfermedad<br>
            <select name="idEn" id="idEn-Modificar">
              <?php
                include 'dbh.php';
                $query = "SELECT * FROM tipoenfermedad ORDER BY nombre";
                $result = mysqli_query($conn, $query) or die;
  ...
  <div id="ModalEliminar" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form id="FormEliminar" action="server/eliminarEnfermedad.php" method="POST">
            <input type="hidden" id="idEnfermedad-Eliminar" name="idEnfermedad" value="">
          </form>
  ...
  <script type="text/javascript">
    function eliminarEnfermedad(idEnfermedad) {
      $("#idEnfermedad-Eliminar").val(idEnfermedad);
    }

    function modificarEnfermedad(idEnfermedad, idEn, nombre) {
      $("#idEnfermedad-Modificar").val(idEnfermedad);
      $("#idEn-Modificar").val(idEn);
